# src/train.py (version GitHub Actions compatible) 
import pandas as pd 
...
# Définir un dossier relatif pour MLflow (évite les chemins Windows)
...
def train_model(model_name, pipeline): 
    ...
    with mlflow.start_run(run_name=model_name): 
        ...
        # Exemple pour input_example MLflow
        example = pd.DataFrame({"text": ["Ceci est un exemple de texte."]})

        # Logger le modèle comme artefact dans mlruns/ relatif
...
if __name__ == "__main__":
    ...
    train_model(model_name, lr_pipeline)
    ...
    train_model(model_name, nb_pipeline)
